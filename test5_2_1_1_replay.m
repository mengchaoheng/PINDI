...
if(isfield(log.data, 'allocation_value_0'))
    allocation_value_table=log.data.allocation_value_0(1:rate_dowm_simple:end, :);
    allocation_value=allocation_value_table{:,:};
    allocation_time=topic_field(allocation_value_table, 'timestamp');
    allocation_u0=topic_field(allocation_value_table, 'u[0]');
    allocation_u_ultimate0=topic_field(allocation_value_table, 'u_ultimate[0]');
    ...
    for i=1:allocation_value_N-1
        allocation_value_delta_t(i)=(allocation_time(i+1))*1e-6-(allocation_time(i))*1e-6;
    end
    for i=1:allocation_value_N-1
        allocation_value_delta(i)=allocation_u0(i+1)-allocation_u0(i);
    end
end 
...
region_edges = [
    flag(2)*1e-6,  flag(6)*1e-6;
    flag(6)*1e-6,  flag(9)*1e-6;
    flag(9)*1e-6,  flag(12)*1e-6
];
region_colors = [
    0.975 0.985 1.000;
    0.990 0.985 0.965;
...
time_start=30;
time_len=175; % manually adjust to hide the later recovery segment
...
text(flag(2)*1e-6+0.3, -22,'$USE\_TAU=1$','Color',wc1_color,'FontSize',8,'Interpreter', 'latex');
...
text(flag(8)*1e-6+0.3, -22,'$f_{c2}=5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(9) flag(9)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(9)*1e-6+0.3, -22,'$f_{c2}=1$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(10) flag(10)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(10)*1e-6+0.3, -22,'$f_{c2}=0.5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(11) flag(11)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(11)*1e-6+0.3, -22,'$f_{c2}=0.3$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(12) flag(12)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(12)*1e-6+0.3, -22,'$u_0=0$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');
...
%标识：
plot([flag(2) flag(2)]*1e-6,[-180 180], '-.', 'Color',wc1_color, 'LineWidth', 0.5);hold on;
text(flag(2)*1e-6+0.3, -62,'$USE\_TAU=1$','Color',wc1_color,'FontSize',8,'Interpreter', 'latex');
...
plot([flag(8) flag(8)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(8)*1e-6+0.3, -62,'$f_{c2}=5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(9) flag(9)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(9)*1e-6+0.3, -62,'$f_{c2}=1$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(10) flag(10)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(10)*1e-6+0.3, -62,'$f_{c2}=0.5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(11) flag(11)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(11)*1e-6+0.3, -62,'$f_{c2}=0.3$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(12) flag(12)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5);hold on;
text(flag(12)*1e-6+0.3, -62,'$u_0=0$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');
...
%标识：
plot([flag(2) flag(2)]*1e-6,[-180 180], '-.', 'Color',wc1_color, 'LineWidth', 0.5');hold on;
text(flag(2)*1e-6+0.3, -15,'$USE\_TAU=1$','Color',wc1_color,'FontSize',8,'Interpreter', 'latex');
...
plot([flag(8) flag(8)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(8)*1e-6+0.3, -15,'$f_{c2}=5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(9) flag(9)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(9)*1e-6+0.3, -15,'$f_{c2}=1$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(10) flag(10)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(10)*1e-6+0.3, -15,'$f_{c2}=0.5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(11) flag(11)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(11)*1e-6+0.3, -15,'$f_{c2}=0.3$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(12) flag(12)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(12)*1e-6+0.3, -15,'$u_0=0$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');
...
% PlotToFileColorPDF(fig1, 'results/Figure_9_fix.pdf', 13.5, 11.5); %states_with_noise
PlotToFileColorPNG(fig1, 'results/states_with_noise_fix.png', 25, 20);
...
text(flag(2)*1e-6+0.3, -30,'$USE\_TAU=1$','Color',wc1_color,'FontSize',8,'Interpreter', 'latex');
...
plot([flag(8) flag(8)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(8)*1e-6+0.3, -30,'$f_{c2}=5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(9) flag(9)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(9)*1e-6+0.3, -30,'$f_{c2}=1$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(10) flag(10)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(10)*1e-6+0.3, -30,'$f_{c2}=0.5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(11) flag(11)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(11)*1e-6+0.3, -30,'$f_{c2}=0.3$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(12) flag(12)]*1e-6,[-180 180], '-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(12)*1e-6+0.3, -30,'$u_0=0$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');
...
plot(allocation_time*1e-6, allocation_u_ultimate0*r2d, '--','Color',control_color, 'LineWidth', 0.7); hold on;% allocation_value \in [-0.3491 0.3491] 
% pwm_max=2000;
% pwm_min=1000;
...
text(flag(2)*1e-6+0.3, -25,'$USE\_TAU=1$','Color',wc1_color,'FontSize',8,'Interpreter', 'latex');
...
text(flag(8)*1e-6+0.3, -25,'$f_{c2}=5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(9) flag(9)]*1e-6,[-30 30],'-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(9)*1e-6+0.3, -25,'$f_{c2}=1$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(10) flag(10)]*1e-6,[-30 30],'-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(10)*1e-6+0.3, -25,'$f_{c2}=0.5$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(11) flag(11)]*1e-6,[-30 30],'-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(11)*1e-6+0.3, -25,'$f_{c2}=0.3$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');

plot([flag(12) flag(12)]*1e-6,[-30 30],'-.', 'Color',wc2_color, 'LineWidth', 0.5');hold on;
text(flag(12)*1e-6+0.3, -25,'$u_0=0$','Color',wc2_color,'FontSize',8,'Interpreter', 'latex');
...
% PlotToFileColorPDF(fig2, 'results/Figure_10_fix.pdf', 13.5, 11.5); % control_with_noise
PlotToFileColorPNG(fig2, 'results/control_with_noise_fix.png', 25, 20);
...
% Get segment indices
idx1 = find(t_us >= time_start*1e6 & t_us <= flag(2));
idx2 = find(t_us >= flag(2) & t_us <= flag(6));
idx3a = find(t_us >= flag(6) & t_us <= flag(9));  % 20, 8, 5 Hz
idx3b = find(t_us >= flag(9) & t_us <= flag(12)); % 1, 0.5, 0.3 Hz
idx4 = find(t_us >= flag(12) & t_us <= time_len*1e6);
...
if ~isempty(idx3a)
    plot(XYZ(idx3a,1), XYZ(idx3a,2), '-', ...
        'Color', wc2_color, 'LineWidth', 0.75);
end
if ~isempty(idx3b)
    plot(XYZ(idx3b,1), XYZ(idx3b,2), '-', ...
        'Color', stage3b_color, 'LineWidth', 1);
end
if ~isempty(idx4)
...
legend('Setpoint', 'Stage 1', 'Stage 2', ...
       'Stage 3a: $f_{c2}=20,8,5$', ...
       'Stage 3b: $f_{c2}=1,0.5,0.3$', 'Stage 4', ...
       'Interpreter', 'latex', ...
       'Location', 'southeast', ...
       'NumColumns', 2,...
       'FontSize', 8);
...
% PlotToFileColorPDF(fig3, 'results/Figure_11_fix.pdf', 7.5, 7.5); % noise_test_position
PlotToFileColorPNG(fig3, 'results/noise_test_position_fix.png', 20, 15);
...
% Read a ULog field by its name so added message fields do not shift columns.
