...
event = [flag(3), flag(6), flag(9), flag(12)];
step_time = 2e6;  % μs
Time = 5 * step_time;
...
% update colors
ref_color  = [0.93, 0.70, 0.15];   % Ref -
inv_color  = [0.25, 0.47, 0.85];   % INV -
dir_color  = [0.60, 0.32, 0.75];   % DIR - 
pca_color  = [0.80, 0.40, 0.10];   % PCA -
wls_color  = [0.25, 0.25, 0.25];   % WLS - 
%%  
set(groot, ...
...
axis([0 7 -40 40]); xticks(0:1:7);yticks(-40:20:40);
...
axis([0 7 -180 180]); xticks(0:1:7);yticks(-180:90:180);


% sgtitle('Angle velocity tracking error under different control', 'FontSize', 8,'FontName','Times New Roman');


PlotToFileColorPDF(fig1,'results/Figure_14a_for_rate_loop_alone.pdf',6.5,10.5); % rate_tracking
PlotToFileColorPNG(fig1,'results/Figure_14a_for_rate_loop_alone.png',6.5,10.5); 
%% Attitude
% Time axis (μs)
...
ylabel('Roll (deg)');
xlabel('Time (s)');
axis([0 7 -20 20]); xticks(0:1:7);yticks(-20:10:20);
...
ylabel('Pitch (deg)');
xlabel('Time (s)');
axis([0 7 -10 10]); xticks(0:1:7);yticks(-10:5:10);
...
ylabel('Yaw (deg)');
xlabel('Time (s)');
axis([0 7 -270 90]); xticks(0:1:7);yticks(-270:90:90);

% sgtitle('Euler angle tracking error under different control strategies', ...
        % 'FontSize', 8, 'FontName', 'Times New Roman');

PlotToFileColorPDF(fig2,'results/Figure_14b_for_rate_loop_alone.pdf',6.5,10.5); % att_tracking
PlotToFileColorPNG(fig2,'results/Figure_14b_for_rate_loop_alone.png',6.5,10.5); % att_tracking

t_alloc = allocation_value(:,1);            % μs
alloc_err = allocation_value(:,3:5);        % Three-axis allocation error
...
ylabel('Roll channel');
axis([0 7 -15 5]); xticks(0:1:7);yticks(-15:5:5);
legend('INDI+INV','INDI+WLS','INDI+DIR','PINDI', ...
    'NumColumns', 2, 'Location', 'southeast', 'FontSize', 6, 'FontName', 'Times New Roman');
...
axis([0 7 -5 10]); xticks(0:1:7);yticks(-5:5:10);
...
ylabel('Yaw channel');
axis([0 7 -40 20]); xticks(0:1:7);yticks(-40:20:20);

% sgtitle('Allocation error $\epsilon_{ca}$', 'Interpreter', 'latex', ...
%         'FontSize', 8, 'FontName', 'Times New Roman');

PlotToFileColorPDF(fig3,'results/Figure_15a_for_rate_loop_alone.pdf',6.5,15.5); % Alloc_error
PlotToFileColorPNG(fig3,'results/Figure_15a_for_rate_loop_alone.png',6.5,15.5); % Alloc_error
...
PlotToFileColorPDF(fig4,'results/Figure_15b_for_rate_loop_alone.pdf',6.5,15.5); % Alloc_u
PlotToFileColorPNG(fig4,'results/Figure_15b_for_rate_loop_alone.png',6.5,15.5); % Alloc_u
...
% Initialize position error results
% err_pos = cell(n_event,1);
% t_rel_pos = cell(n_event,1);

pos = cell(n_event,1);
t_pos = cell(n_event,1);

% === Interpolation + Error calculation: Position ===
for k = 1:n_event
    t0 = event(k) + 0.5 * step_time;
    t1 = t0 + Time - 1.5* step_time;

    % Current window data (Position)
...
PlotToFileColorPDF(fig5, 'results/Figure_16_for_rate_loop_alone.pdf', 7.5, 7.5); % position_tracking
PlotToFileColorPNG(fig5, 'results/Figure_16_for_rate_loop_alone.png', 7.5, 7.5); 
