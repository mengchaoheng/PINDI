function ratio = vview_compare(B, plim, P, v_1, v_2,v_all, v_21,v_all1,v_22,v_all2)
...
v_22_color = [0.70, 0.50, 0.00];       
  ...
  switch k
   ...
   case 2
    ...
    % Initialize the legend object array and label array
legend_handles = [];
legend_labels  = [];
...
if exist('v_1','var') && ~isempty(v_1)
    h = quiver(0, 0, v_1(1), v_1(2), 0, '-','Color', AMS_edge_color, ...
        'LineWidth', 1, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu_h$';
end
if exist('v_2','var') && ~isempty(v_2)
    h = quiver(v_1(1), v_1(2), v_2(1), v_2(2), 0, '-', 'Color', INV_edge_color,...
        'LineWidth', 1, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu_c$';
end
if exist('v_all','var') && ~isempty(v_all)
    h = quiver(0, 0, v_all(1), v_all(2), 0, '-','Color', v_color, ...
        'LineWidth', 0.5, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu$';
end

if exist('v_21','var') && ~isempty(v_21)
    h = quiver(0, 0, v_21(1), v_21(2), 0, '-', 'Color', v_1_color,...
        'LineWidth', 1, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu_{\mathrm{inv}}$';
end
if exist('v_all1','var') && ~isempty(v_all1)
    h = quiver(0, 0, v_all1(1), v_all1(2), 0, '-','Color', v_2_color, ...
        'LineWidth', 1, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu_{\mathrm{wls}}$';
...
if exist('v_22','var') && ~isempty(v_22)
    h = quiver(0, 0, v_22(1), v_22(2), 0, '-','Color', v_21_color, ...
        'LineWidth', 1, 'MaxHeadSize', 0.3, 'AutoScale', 'off');
    legend_handles(end+1) = h;
    legend_labels{end+1}  = '$\nu_{\mathrm{dir}}$';
end
...
%  
legend(legend_handles, legend_labels, 'Location', 'southwest', 'NumColumns', 3,'Interpreter', 'latex', 'FontSize', 8);
